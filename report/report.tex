...
\bottomrule
\end{tabular}
\caption{Test-set reconstruction metrics on held-out test images.}
\label{tab:metrics}
\end{table}

The AE achieves lower MSE on all six classes, as expected from its direct pixel-fidelity
objective. SSIM tells a more nuanced story: the AE leads on AbdomenCT, CXR, ChestCT,
and Hand, but the VAE matches or exceeds it on BreastMRI and HeadCT -- classes with
higher intra-class variability, where the VAE's prior regularisation suppresses
over-fitting artefacts. The VAE also supports \emph{generation} from the prior (see
\texttt{figures/vae\_samples\_global.png}) and smooth interpolation. For
\emph{denoising}, both models improve on the noisy input at
$\sigma\!\in\!\{0.1,0.2,0.3\}$; the VAE degrades more gracefully at high $\sigma$
because its prior-regularized decoder resists over-fitting to noise patterns.
